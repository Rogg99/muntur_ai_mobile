...
class Chat_ extends State<ChatView>  with TickerProviderStateMixin{
                ...
                children: [
                      ...
                      children: [
                        ...
                        Align(
                          ...
                          Container(
                            ...
                            child: Row(
                              children: [
                                Text(getDuration(videoController.value.position.inMilliseconds),style: appStyle.txtRoboto(size: 18,color: Colors.white),),
                                Flexible(
                                ...
                                Text(getDuration(videoController.value.duration.inMilliseconds),style: appStyle.txtRoboto(size: 18,color: Colors.white),),
                              ],
                            ),
                          ),
                        )
                  ...
                  Center(
                    child: GestureDetector(
                      ...
                      child: videoController.value.isPlaying ? const SizedBox():const Icon(Icons.play_circle,size: 60,color: Colors.white,),
                    ),
                  ),
                ],
              ):
                      ...
                      Stack(
                        children: [
                          GestureDetector(
                            onTap: (){
                              //setState(() {});
                              scrollcontroller.scrollToIndex(getIndexOfmessage(answerTo.id), preferPosition: AutoScrollPosition.begin,duration: const Duration(seconds: 1));
                                ...
                                Row(
                                  children: [
                                    Column(
                                      mainAxisSize: MainAxisSize.min,
                                      children: [
                                        Text(answerTo.emetteur == user!.id? translator.you : answerTo.emetteurName,
  ...
  syncView(){
    ...
    showmedia=false;
    media = '';
    scrollcontroller.scrollToIndex(messages.length-1, preferPosition: AutoScrollPosition.end,duration: const Duration(milliseconds: 1000));
    if(mounted) {
      FocusScope.of(context).requestFocus(FocusNode());
    }
  }

  sendMessage(UIMessage message, BuildContext context,{bool start=true}) async {
    ...
    if (start) {
      ...
      {
        if (kDebugMode)
        print('Muntur DEBUG: starting discussion response ${startDiscResp.body.toString()}'),
  ...
  sendQuestion(BuildContext context) async {
    ...
    if(messagecontroller.text.isNotEmpty || media.isNotEmpty) {
      ...
      String timeFormat = DateTime.now().format('yyyy-MM-dd HH:mm');
      if(media.isNotEmpty){
        String newName='muntur${time}_${user!.id.replaceAll(':', '').replaceAll('-', '')}.${media.split('.').last}';
