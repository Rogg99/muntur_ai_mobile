...
final ValueNotifier<List<Discussion>> forums = ValueNotifier([]);
...
class StateHomeScreen extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  bool isLoading = true,isLoadingForums = true,searchdone=true,loadGarages = true ,showGarages = true;
  ...
  Widget build(BuildContext context) {
    ...
    List<String> _titlesView = [translator.homeTitle,translator.searchTitle, translator.newsTitle, translator.forumsTitle,translator.settings_title];
    List<Widget> _widgetOptions = <Widget>[
      Scaffold(
                  ...
                  Center(
                    child: Padding(
                      ...
                      child: Text('You haven\'t started no chat yet, start a new one to enjoy MUNTUR AI !!!',
                        style: appStyle.H4(color: Theme.of(context).colorScheme.primary),
        ...
        floatingActionButton: FloatingActionButton.extended(
          onPressed: (){
            Navigator.push(context, MaterialPageRoute(builder: (context) => ChatView(disc: Discussion(),)));
          },
          backgroundColor: Theme.of(context).colorScheme.primary,
          label: Text(translator.newDiscussion,style: const TextStyle(color: Colors.white),),
          ...
          children: [
            SizedBox(
              ...
              child: FlutterMap(
                ...
                children: [
                  TileLayer(
                    urlTemplate: 'https://mt1.google.com/vt/lyrs=m&x={x}&y={y}&z={z}&key='+LocalisationApi().GOOGLE_MAPS_APIKEY,
                ...
                Stack(
                  children: [
                    TextField(
                      ...
                      onSubmitted: (value){
                        fetchgarages(garageSearchController.value.text,context);
                      },
                    ),
                        ...
                        child: Padding(
                          ...
                          child: GestureDetector(
                            onTap: (){
                              ...
                              child: Icon(Icons.search,color: Theme.of(context).colorScheme.background,size: 32,)
                          ),
                        )),
                  ],
                ),
                      ...
                      child: Row(
                        ...
                        children: [
                          GestureDetector(
                              ...
                              child: Text('Results',style: appStyle.H4(weight: 'bold'),)
                          ),
                        ],
                      ),
                    ),
                      ...
                      ListView(
                        children: [
                          ...
                          for(var i=0;i<garages.length;i++)
                            widgetGarage(garage: garages[i],parent:this),
      ...
      Infos(),
      SizedBox.expand(
        child:  isLoadingForums ?
        Center(
            child: CircularProgressIndicator(
              color: Theme.of(context).colorScheme.primary,
            )):
        (isLoadingForums==false && forums.value.isEmpty)?
        Center(
          child: Padding(
            padding: const EdgeInsets.all(8.0),
            child: Text('No forums available yet !',
              style: appStyle.H4(color: Theme.of(context).colorScheme.primary),
              textAlign: TextAlign.center,
            ),
          ),
        ...
        Container(
            ...
            ListView.separated(
              ...
              itemBuilder: (context,index){
                return
                  WidgetForum(
                    disc: forums.value[index],
                  );
              }, itemCount: forums.value.length,
            )
        ),
      ),
      Settings(),
  ...
  Future<bool> _onBackPressed() async {
    ...
    if (firstclick == false) {
      firstclick = true;
      toast('clic again to leave',color: Theme.of(context).colorScheme.background);
      return false;
    } else {
      return true;
    }
  ...
  Future<List<Discussion>> fetchDiscussions() async {
    ...
    // var different=false;
    await DatabaseHelper().getDiscussion().then((list) => {
        // log("discussions count : ${list.length}"),
        // reorder the list by last_date descending
        list.sort((a, b) => DateTime.parse(b.last_date).isAfter(DateTime.parse(a.last_date)) ? 1 : -1),
          ...
          for (var i = 0; i < datas.length; i++)
            {
              garages_.add(Garage(
                ...
                photo: datas[i]['photo']!='none' ?datas[i]['photo']:'none',
          ...
          child: Column(
            children: [
              Padding(
                padding: const EdgeInsets.all(8.0),
                child: Container(
                    child: const Center(child: Icon(Icons.man,color: Colors.blue,size: 36,))),
              ),
              Text('You',
  ...
  fetchforums(BuildContext context) async {
    isLoadingForums = true;
    List<Discussion> datas=[];
    var results;
    await ChatApi().getForums().then((response) => {
      if (response.statusCode == 200)
        {
          results = json.decode(response.body),
          ...
          for(var item in results){
            datas.add(
              Discussion(
                id: item["id"],
                title: item['titre'],
                last_message: item['last_message']['contenu'],
                last_date: (item['last_message']['date_creation'] as String).substring(0,10),
                last_writer: item['last_message']['emetteur']['nom'],
                type: "forum",
              ),
            )
          },
        ...
        toast("Oups! An error occured during the fetch , Please try again !",color: Colors.redAccent),
        print(response.body)
      }
