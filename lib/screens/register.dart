...
class _signupState extends ConsumerState<Signup2>
  ...
  bool sexe = false, privacyPolicy = false, show_loading = false;
        ...
        body: ListView(
          ...
          children: [
            ...
            Text(
              ...
              style: appStyle
                  .H6()
                  .copyWith(fontStyle: FontStyle.italic, color: textFieldColor),
            ),
            Padding(padding: getPadding(top: 15)),
            PrimaryButton(
  ...
  Future<void> register() async {
    ...
    if (pwdcontroller.text != pwdCfcontroller.text) {
      setState(() {
        ...
        textFieldMessage =
            "Password not matching, Please fill correctly the cases";
        textFieldColor = Colors.redAccent;
        show_loading = false;
      });
      return;
    ...
    try {
      if (birthdaycontroller.text.isEmpty ||
          int.parse(birthdaycontroller.text) > 31 ||
          int.parse(birthdaycontroller.text) < 0) {
        setState(() {
          dayFieldColor = Colors.redAccent;
          textFieldMessage =
              "Please fill correctly the case Day of birth date*";
          textFieldColor = Colors.redAccent;
          show_loading = false;
        });
        return;
      }
      if (birthmonthcontroller.text.isEmpty ||
          int.parse(birthmonthcontroller.text) > 12 ||
          int.parse(birthmonthcontroller.text) < 0) {
        setState(() {
          monthFieldColor = Colors.redAccent;
          textFieldMessage =
              "Please fill correctly the case Month of birth date*";
          textFieldColor = Colors.redAccent;
          show_loading = false;
        });
        return;
      ...
      if (birthyearcontroller.text.isEmpty ||
          int.parse(birthyearcontroller.text) < 1950 ||
          int.parse(birthyearcontroller.text) > DateTime.now().year - 14) {
        setState(() {
          yearFieldColor = Colors.redAccent;
          textFieldMessage =
              "Please fill correctly the case year of birth date*";
          textFieldColor = Colors.redAccent;
          show_loading = false;
        });
        return;
    ...
    if (!validDate(
        "${birthdaycontroller.text}/${birthmonthcontroller.text}/${birthyearcontroller.text}")) {
      setState(() {
        dayFieldColor = Colors.redAccent;
        textFieldMessage =
            "Please fill correctly the cases of birth date* format jj/mm/aaaa ex 12/10/1995";
        textFieldColor = Colors.redAccent;
        show_loading = false;
      });
      return;
    ...
    try {
      await ref.read(authStateProvider.notifier).register(inscription.toMap());
      if (!mounted) return;
      ...
      toast('Sign up failed', color: Colors.grey);
      return;
    } finally {
      if (mounted) {
        setState(() {
